...
export const PuzzleGame: React.FC<PuzzleGameProps> = () => {
  ...
  const puzzleGame = usePuzzleGame({
    ...
    onGameSave: (saveData) => {
      ...
      if (gameSettings.autoSave) {
        localStorage.setItem(`puzzle_save_${id}`, JSON.stringify(saveData))
      }
    },
    onError: (error) => {
  ...
  const loadPuzzleData = async () => {
    try {
      // 저장된 게임이 있는지 확인
      const savedGame = localStorage.getItem(`puzzle_save_${id}`)
      if (savedGame && gameSettings.autoSave) {
        const saveData = JSON.parse(savedGame)
        await puzzleGame.loadSavedGame(saveData)
      } else {
        await puzzleGame.loadPuzzle(id!)
      }
    } catch (error) {
